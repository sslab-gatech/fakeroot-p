...
static void read_real_uid() { read_id(&faked_real_uid, FAKEROOTUID_ENV); }
static void read_effective_uid()
{
    read_id(&faked_effective_uid, FAKEROOTEUID_ENV);
}

static void read_saved_uid() { read_id(&faked_saved_uid, FAKEROOTSUID_ENV); }
static void read_fs_uid() { read_id(&faked_fs_uid, FAKEROOTFUID_ENV); }
...
static void read_real_gid() { read_id(&faked_real_gid, FAKEROOTGID_ENV); }
static void read_effective_gid()
{
    read_id(&faked_effective_gid, FAKEROOTEGID_ENV);
}

static void read_saved_gid() { read_id(&faked_saved_gid, FAKEROOTSGID_ENV); }
static void read_fs_gid() { read_id(&faked_fs_gid, FAKEROOTFGID_ENV); }
...
static int write_fs_uid() { return write_id(FAKEROOTFUID_ENV, faked_fs_uid); }
...
static int write_fs_gid() { return write_id(FAKEROOTFGID_ENV, faked_fs_gid); }
...
int WRAP_LSTAT LSTAT_ARG(int ver, const char *file_name, struct stat *statbuf)
{
    int r;
...
int WRAP_STAT STAT_ARG(int ver, const char *file_name, struct stat *st)
{
    int r;
...
int WRAP_FSTAT FSTAT_ARG(int ver, int fd, struct stat *st)
{
    int r;
...
#ifdef HAVE_FSTATAT
int WRAP_FSTATAT FSTATAT_ARG(int ver, int dir_fd, const char *path,
...
int WRAP_LSTAT64 LSTAT64_ARG(int ver, const char *file_name, struct stat64 *st)
{
    int r;
...
int WRAP_STAT64 STAT64_ARG(int ver, const char *file_name, struct stat64 *st)
{
    int r;
...
int WRAP_FSTAT64 FSTAT64_ARG(int ver, int fd, struct stat64 *st)
{
    int r;
...
#ifdef HAVE_FSTATAT
int WRAP_FSTATAT64 FSTATAT64_ARG(int ver, int dir_fd, const char *path,
...
int WRAP_MKNOD MKNOD_ARG(int ver UNUSED, const char *pathname, mode_t mode,
                         dev_t XMKNOD_FRTH_ARG dev)
{
    INT_STRUCT_STAT st;
...
#ifdef HAVE_FSTATAT
#ifdef HAVE_MKNODAT
int WRAP_MKNODAT MKNODAT_ARG(int ver UNUSED, int dir_fd, const char *pathname,
...
int unlinkat(int dir_fd, const char *pathname, int flags)
{
    int r;
    INT_STRUCT_STAT st;
    r = INT_NEXT_FSTATAT(dir_fd, pathname, &st,
                         (flags & ~AT_REMOVEDIR) | AT_SYMLINK_NOFOLLOW);
    if (r)
        return -1;
...
#ifdef HAVE_FSTATAT
#ifdef HAVE_RENAMEAT
int renameat(int olddir_fd, const char *oldpath, int newdir_fd,
             const char *newpath)
{
    int r, s;
    INT_STRUCT_STAT st;
...
static size_t common_setxattr(INT_STRUCT_STAT *st, const char *name,
...
#endif /* defined(HAVE_SETXATTR) || defined(HAVE_LSETXATTR) ||                 \
          defined(HAVE_FSETXATTR) */

#if defined(HAVE_GETXATTR) || defined(HAVE_LGETXATTR) || defined(HAVE_FGETXATTR)
static size_t common_getxattr(INT_STRUCT_STAT *st, const char *name,
...
#endif /* defined(HAVE_GETXATTR) || defined(HAVE_LGETXATTR) ||                 \
             ...
             size_t __size, int __flags)
{
    ...
    int r;
    if (fakeroot_disabled)
        return next_setxattr(__path, __name, (void *)__value, __size, __flags);
...
#endif /* LIBFAKEROOT_DEBUGGING */
    r = INT_NEXT_STAT(__path, &st);
    if (r)
        return r;

    return common_setxattr(&st, __name, (void *)__value, __size, __flags);
}
#endif /* HAVE_SETXATTR */
              ...
              size_t __size, int __flags)
{
    ...
    int r;
    if (fakeroot_disabled)
        return next_lsetxattr(__path, __name, (void *)__value, __size, __flags);
              ...
              int flags)
{
    ...
    int r;
    if (fakeroot_disabled)
        return next_fsetxattr(fd, name, (void *)value, size, flags);
    ...
    return common_setxattr(&st, name, (void *)value, size, flags);
}
...
int fakeroot_isdisabled(void) { return fakeroot_disabled; }
#ifdef HAVE_ACL_T
...
FTSENT *fts_read(FTS *ftsp)
{
    ...
    if (r && ((ftsp->fts_options & FTS_NOSTAT) || r->fts_info == FTS_NS ||
              r->fts_info == FTS_NSOK))
...
#ifdef __LP64__
int fgetattrlist(int fd, void *attrList, void *attrBuf, size_t attrBufSize,
    ...
    for (i = 0; i < envcount; ++i) {
        if (strncmp(altenv[i], "DYLD_INSERT_LIBRARIES=", 22) == 0) {
            if (strstr(altenv[i], dylib_insert))
                ...
                *insenvp = (char *)malloc(strlen(altenv[i]) +
                                          strlen(dylib_insert) + 2);
                if (!*insenvp) {
                    free(altenv);
                    return (char **)envp;
                }
...
static char *find_exec_path(const char *file, const char *search_path)
{
    ...
    if (file && *file && !strchr(file, '/') && search_path && *search_path) {
        ...
        for (; search_path < search_end; search_path = next) {
            ...
            if (stat(buff, &info) || !S_ISREG(info.st_mode) ||
                access(buff, X_OK))
                continue;
            path = buff;
            break;
                 ...
                 char *const argv[__restrict], char *const envp[__restrict])
{
    ...
    char *path =
        find_exec_path(file, search_path ? search_path : _PATH_DEFPATH);
    if (!path)
        return errno;
