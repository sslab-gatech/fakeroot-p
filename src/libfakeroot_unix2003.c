...
extern int set_faked_reuid(uid_t ruid, uid_t euid)
...
extern int set_faked_regid(gid_t rgid, gid_t egid)
    __attribute__((visibility("hidden")));
...
int getattrlist$UNIX2003(const char *path, void *attrList, void *attrBuf,
...
#endif /* LIBFAKEROOT_DEBUGGING */
    r = next_getattrlist$UNIX2003(path, attrList, attrBuf, attrBufSize,
                                  options);
    if (r) {
        return r;
    }
