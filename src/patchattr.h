...
static void patchattr(void *attrList, void *attrBuf, uid_t uid, gid_t gid,
                      mode_t mode)
{
        ...
        if (l->commonattr & attrs[i].value) {
            ...
            if (fakeroot_debug) {
                ...
                if (attrs[i].isRef) {
                    ...
                    size_t begin =
                        here + ((attrreference_t *)b)->attr_dataoffset;
                    size_t size = ((attrreference_t *)b)->attr_length;
                    size_t alignedEnd = (begin + size + 3) & ~3;
                    fprintf(stderr, "patchattr reference begin %zu size %zu "
                                    "aligned end %zu\n",
