...
static void xattr_fill(xattr_node_t *node, char *key, char *value,
                       size_t value_size)
{
    if (node->key)
        free(node->key);
...
/*
 * XXX: Will never scale, single linked list for the hash table
 */
static data_node_t *data_find(const struct fakestat *key, const uint32_t remote)
{
    data_node_t *n;
...
static void data_insert(const struct fakestat *buf, const uint32_t remote)
{
...
static void sd_list_add(int sd)
{
    if (sd_list.capacity == sd_list.size) {
        ...
        if (sd_list.array == NULL) {
            ...
            sd_list.array =
                realloc(sd_list.array, sd_list.capacity * sizeof(int));
            if (!sd_list.array)
                fail("realloc");
        }
    }
    ...
    for (i = data_begin(); i != data_end(); i = data_node_next(i)) {
        ...
        if (find_path(i->buf.dev, i->buf.ino, roots, path))
            fprintf(f, "mode=%llo,uid=%llu,gid=%llu,nlink=%llu,rdev=%llu %s\n",
                    (uint64_t)i->buf.mode, (uint64_t)i->buf.uid,
                    (uint64_t)i->buf.gid, (uint64_t)i->buf.nlink,
                    (uint64_t)i->buf.rdev, path);
#else
        fprintf(f, "dev=%llx,ino=%llu,mode=%llo,uid=%llu,gid=%llu,nlink=%llu,"
                   "rdev=%llu\n",
                (uint64_t)i->buf.dev, (uint64_t)i->buf.ino,
                (uint64_t)i->buf.mode, (uint64_t)i->buf.uid,
                (uint64_t)i->buf.gid, (uint64_t)i->buf.nlink,
                (uint64_t)i->buf.rdev);
#endif
...
#ifdef FAKEROOT_DB_PATH
        r = scanf(
            "mode=%llo,uid=%llu,gid=%llu,nlink=%llu,rdev=%llu " DB_PATH_SCAN
            "\n",
...
#else
        r = scanf("dev=%llx,ino=%llu,mode=%llo,uid=%llu,gid=%llu,nlink=%llu,"
                  "rdev=%llu\n",
...
void debug_stat(const struct fakestat *st)
{
    fprintf(
        stderr,
        "dev:ino=(%llx:%lli), mode=0%lo, own=(%li,%li), nlink=%li, rdev=%lli\n",
        st->dev, st->ino, (long)st->mode, (long)st->uid, (long)st->gid,
        (long)st->nlink, st->rdev);
}

void insert_or_overwrite(struct fakestat *st, const uint32_t remote)
{
    data_node_t *i;
...
void process_chmod(struct fake_msg *buf)
{
    ...
    if (i != data_end()) {
        ...
        /* Statically linked binaries can remove inodes without us knowing.
           ...
           While those cases can be fixed in other ways, we shouldn't continue
           to
           cache stale file information.

           mknod() creates a regular file, everything else should have the same
           file type on disk and in our database.  Therefore, we check the
           file's
           type first.  If we have something in our database as a device node
           and
           we get a request to change it to regular file, it might be a chmod of
           a device node that was created from within fakeroot, which is a
           device
           file on disk - there's no way to distinguish.   For anything else, we
           trust the new type and assume the inode got unlinked from something
           that
...
void process_setxattr(struct fake_msg *buf)
{
    ...
    if (x) {
        ...
        if (flags == XATTR_REPLACE) {
            ...
            if (debug) {
                fprintf(stderr, "FAKEROOT:    Replace requested but no "
...
void get_msg()
{
    ...
    do {
        ...
        if (debug) {
            fprintf(stderr,
                    "FAKEROOT: r=%i, received message type=%li, message=%i\n",
                    r, buf.mtype, buf.id);
        }
        if (r != -1) {
            buf.remote = 0;
...
#ifndef FAKEROOT_FAKENET
#define FAKEROOT_CLEANUPMSG                                                    \
    "fakeroot: clearing up message queues and semaphores, signal=%i\n"
#else /* FAKEROOT_FAKENET */
...
#else  /* FAKEROOT_FAKENET */
            fprintf(stderr, "This fakeroot has been compiled for TCP and does "
                            "not support --key\n");
#endif /* FAKEROOT_FAKENET */
...
#ifndef FAKEROOT_FAKENET
            fprintf(stderr, "This fakeroot has been compiled for SYSV IPC and "
...
#ifndef FAKEROOT_FAKENET
            fprintf(stderr, "options for fakeroot: --key, --cleanup, "
                            "--foreground, --debug, --save-file, --load, "
                            "--unknown-is-real\n");
#else  /* FAKEROOT_FAKENET */
            fprintf(stderr, "options for fakeroot: --port, --foreground, "
                            "--debug, --save-file, --load, "
                            "--unknown-is-real\n");
#endif /* FAKEROOT_FAKENET */
            exit(1);
        }
