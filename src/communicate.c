...
void cpyfakefake(struct fakestat *dest, const struct fakestat *source)
{
...
void lock_comm_sd(void) { pthread_mutex_lock(&comm_sd_mutex); }
void unlock_comm_sd(void) { pthread_mutex_unlock(&comm_sd_mutex); }
...
void send_get_fakem(struct fake_msg *buf)
{
    ...
    */
    ...
    if (init_get_msg() != -1) {
        ...
        do
            l = msgrcv(msg_get, (struct my_msgbuf *)buf,
        ...
        printf("libfakeroot/fakeroot, internal bug!! get_fake: length=%i !=
        l=%i",
...
void send_stat(const struct stat *st, func_id_t f
#ifdef STUPID_ALPHA_HACK
               ,
               int ver
#endif
...
#ifdef STAT64_SUPPORT
void send_stat64(const struct stat64 *st, func_id_t f
#ifdef STUPID_ALPHA_HACK
                 ,
                 int ver
#endif
...
void send_get_xattr(struct stat *st, xattr_args *xattr
#ifdef STUPID_ALPHA_HACK
                    ,
                    int ver
#endif
...
void send_get_xattr64(struct stat64 *st, xattr_args *xattr
#ifdef STUPID_ALPHA_HACK
                      ,
                      int ver
#endif
...
int fake_get_owner(int is_lstat, const char *key, const char *path, uid_t *uid,
